...
- `docker-compose.prod.yml` — override that adds `caddy` (`docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d`), used for the prod deploy only. The dev stack runs without it — it's reached through *prod's* Caddy instance instead, over the shared external `caddy_net` network (one public IP, one Caddy, one place TLS certs live).
- `Caddyfile` — routes all four domains, every route by full container name (`<project>-<service>-1`), never the short service name (`frontend`/`api`). Both `my-car` and `my-car-dev` run a service by that short name, and Compose registers it as a DNS alias on *every* network a service joins — including the shared external `caddy_net` — so the short name round-robins between both projects' containers there. Confirmed live (caused a real incident, prod intermittently serving dev's build): `getent hosts api` from inside the Caddy container returned both projects' `api` containers, alternating per query.
